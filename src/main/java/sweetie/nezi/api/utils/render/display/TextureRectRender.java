...
public class TextureRectRender {
    ...
    public void drawHead(MatrixStack matrixStack, PlayerEntity player, float x, float y, float width, float height, float gap, float radius, Color color) {
        ...
        Identifier skin = DefaultSkinHelper.getSkinTextures(player.getGameProfile()).texture();
        if (player == client.player && player instanceof AbstractClientPlayerEntity abstractClientPlayer) {
            try {
                skin = abstractClientPlayer.getSkinTextures().texture();
            } catch (Exception ignored) {
                skin = DefaultSkinHelper.getSkinTextures(player.getGameProfile()).texture();
            }
        }
        float u = 8f / 64f;
        float u2 = 40f / 64f;
        ...
        int texture = client.getTextureManager().getTexture(skin).getGlId();
