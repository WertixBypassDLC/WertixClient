package sweetie.nezi.api.utils.math;
...
public class ProjectionUtil implements QuickImports {
    ...
    public Vector2f project(double x, double y, double z) {
        ...
        // Защита: если камера еще не создана, возвращаем координаты за пределами экрана
        if (camera == null) {
    ...
    public boolean isInFrontOfCamera(double x, double y, double z) {
        var camera = mc.getEntityRenderDispatcher().camera;
        if (camera == null) {
            return false;
        }
        ...
        if (toPoint.lengthSquared() <= 1.0E-6D) {
            return true;
        }

        Vec3d forward = Vec3d.fromPolar(camera.getPitch(), camera.getYaw());
        return forward.dotProduct(toPoint.normalize()) > 0.0D;
    }

    public boolean isProjectedOnScreen(Vector2f projected, float padding) {
        return projected != null
                && projected.x != Float.MAX_VALUE
                && projected.y != Float.MAX_VALUE
                && projected.x >= -padding
                && projected.x <= mc.getWindow().getScaledWidth() + padding
                && projected.y >= -padding
                && projected.y <= mc.getWindow().getScaledHeight() + padding;
    }
