...
import sweetie.nezi.api.utils.render.fonts.MsdfGlyph;
...
public class TextUtil implements QuickImports {
    ...
    private void parseTextRecursive(Text text, int currentColor, List<MsdfGlyph.ColoredGlyph> result) {
        ...
        if (content instanceof PlainTextContent.Literal) {
            raw = ((PlainTextContent.Literal) content).string();
    ...
    // ИЗМЕНЕНИЕ: Сообщения теперь идут в уведомления, а не в чат
    public void sendMessage(String message) {
        // Убираем цветовые коды Minecraft (§), так как уведомления рисуют свой цвет
        String cleanMessage = message.replaceAll("§[0-9a-fk-or]", "");
        NotificationUtil.add(cleanMessage);
    }
