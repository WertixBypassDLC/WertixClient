...
public class ArrayListWidget extends Widget {
    ...
    private boolean prevRightSide = false;
    private float sideAnim = 1f; // 1 = fully settled
    ...
    public void render(MatrixStack ms) {
        ...
        for (Module m : visible) {
            float w = Fonts.PS_MEDIUM.getWidth(m.getLocalizedName(), fS) + p * 2f;
        ...
        for (Module m : visible) {
            float anim  = animMap.getOrDefault(m, 0f);
            float drawY = yAnimMap.getOrDefault(m, y);
            ...
            Fonts.PS_MEDIUM.drawText(ms, name, textX,
                    drawY + rowH / 2f - fS / 2f + scaled(0.3f), fS, widgetTextColor(al));

            ms.pop();
        }

        getDraggable().setWidth(maxW);
        getDraggable().setHeight(cY - y);
    }
}
