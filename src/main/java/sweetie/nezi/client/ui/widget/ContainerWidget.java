...
public abstract class ContainerWidget extends Widget {
    ...
    private float getDefaultHeight() { return getFontSize(true) + getGap() * 2.15f; }
    ...
    public void render(MatrixStack matrixStack) {
        ...
        width = headerWidth + getOffset() * 3.15f;
        height = getDefaultHeight();

        RenderUtil.BLUR_RECT.draw(matrixStack, x, y, animWidth, animHeight, round, UIColors.panel(alpha));
        RenderUtil.BLUR_RECT.draw(matrixStack, x + scaled(1f), y + scaled(1f),
                Math.max(0f, animWidth - scaled(2f)), Math.max(0f, animHeight - scaled(2f)),
                Math.max(1f, round - scaled(0.6f)), UIColors.overlay(alpha));
        RenderUtil.RECT.draw(matrixStack, x, y, animWidth, animHeight, round, UIColors.stroke(alpha));
        getMediumFont().drawCenteredGradientText(matrixStack, getName(), x + animWidth / 2f, y + getDefaultHeight() / 2f - headerSize / 2f, headerSize, UIColors.themeFlow(getName().hashCode(), alpha), UIColors.themeFlowAlt(getName().hashCode(), alpha), headerWidth / 4f);
