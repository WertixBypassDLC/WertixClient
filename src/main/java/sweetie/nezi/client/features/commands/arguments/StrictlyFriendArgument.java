package sweetie.nezi.client.features.commands.arguments;
...
public class StrictlyFriendArgument implements ArgumentType<String> {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestMatching(FriendManager.getInstance().getSortedData(), builder);
    }

    @Override
    public Collection<String> getExamples() {
        List<String> examples = FriendManager.getInstance().getSortedData().stream().limit(5).collect(Collectors.toList());
